#!/bin/bash
# Install Emby Server on RHEL/CentOS/Fedora systems
...
if [ -f /etc/redhat-release ]; then
    yum install -y wget ca-certificates ffmpeg
else
    dnf install -y wget ca-certificates ffmpeg
fi

# Create emby user
if ! id -u emby > /dev/null 2>&1; then
    groupadd --system emby
    useradd --system --ingroup emby emby
fi
